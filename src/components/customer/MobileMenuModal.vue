...
const navLinks = [
  ...
  { to: '/cart', label: `Giỏ hàng (${totalQuantity.value})`, icon: 'cart' }, // Ví dụ thêm icon/badge
]
</script>
  ...
  <transition name="slide-right">
    ...
    >
      <div class="p-4 border-b dark:border-gray-700 flex justify-between items-center">
        ...
        <button v-else @click="openLogin" class="bg-green-600 text-white px-3 py-1 rounded-full text-sm hover:bg-green-700 transition">
          Đăng nhập
        </button>

        <button @click="closeMenu" class="text-gray-500 hover:text-gray-900 dark:hover:text-white transition">
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        >
            <span v-if="link.icon === 'cart'" class="inline-flex items-center gap-2">
                {{ link.label.split(' ')[0] }}
                <span v-if="totalQuantity > 0" class="ml-1 px-2 py-0.5 text-xs font-bold text-white bg-red-500 rounded-full">
                    {{ totalQuantity }}
                </span>
            </span>
        </NavLink>
